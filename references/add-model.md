...
接入要**读写仓库文件、跑脚本**，所以这三个值必须拿到 —— 它们只在**本阶段**与排障阶段出现，
调用阶段一次都不用：

| 值 | 怎么拿 |
|---|---|
| `<ComfyUI>` | 手上只要有 `custom_nodes/<节点>/` 下的任意文件，它的上两级就是它 |
| `<ComfyUI python>` | 按序试 `<ComfyUI>/../python/python.exe`（aki 便携包）→ `<ComfyUI>/../python_embeded/python.exe`（官方 portable）→ `sys.executable` |
| 项目根 | `<ComfyUI>/custom_nodes/ComfyUI-Roundabout/` |

取到的值就近记下即可，**不要写回本 skill** —— skill 要能跨机器用。

> **MCP 工具在手时不需要任何地址**：工具是本节点自己注册的，节点在跑就说明服务在。
> 只有当你**没有 MCP 通道**、要走 REST 时才需要端口 —— 默认 `8188`，REST `/v1/*` 与 MCP `/mcp` 同端口。
> **别硬编内网地址。**
